        ...
        <NuxtLink class="absolute inset-0 bg-gradient-to-t from-black/50 hover:from-black/60 flex items-end p-5" :to="`/product/${product.slug}-${product.sku.split('-')[0]}`">
          <div class="grid gap-0.5 text-white">
            <div class="flex gap-2 text-sm">
              <span v-if="product.salePrice && parseFloat(product.salePrice) < parseFloat(product.regularPrice)" v-html="product.salePrice"></span>
              <span v-if="product.salePrice && parseFloat(product.salePrice) < parseFloat(product.regularPrice)" class="line-through text-white/50" v-html="product.regularPrice"></span>
              <span v-else v-html="product.regularPrice"></span>
            </div>
            <div class="font-bold">{{ product.name }}</div>
            <div class="text-sm font-medium">{{ product.productCategories?.nodes?.[0]?.name || '' }}</div>
          </div>
        </NuxtLink>
      </div>
